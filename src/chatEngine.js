  ...
  async init() {
    ...
    if (!this.model.isCached) {
      ...
      await this.model.download((progress) => {
        const normalizedProgress = Math.max(0, Math.min(1, progress > 1 ? progress / 100 : progress));
        const pct = Math.round(normalizedProgress * 100);
        this._emitStatus("download", `Downloading ${this.modelAlias}... ${pct}%`, normalizedProgress);
      });
