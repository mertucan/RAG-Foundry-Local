...
class EvaluationTests(unittest.TestCase):
    def test_evaluate_question_marks_expected_doc_hit(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                store.insert("DOC-AI-001", "RAG Notes", "AI Concepts", 0, "rag retrieval notes", [1.0, 0.0])
                ...
                result = evaluate_question(
                    ...
                    {
                        "id": "EVAL-001",
                        "question": "What are the key ideas in RAG?",
                        "expected_doc_ids": ["DOC-AI-001"],
                    },
                    top_k=1,
                )
                self.assertTrue(result["hit"])
                self.assertEqual(result["retrieved_doc_ids"], ["DOC-AI-001"])
            finally:
                store.close()
